...
$possible_plugin_locations = array(
	...
	'/home/clients/550ab55aa9f1af60db401dfade345bcc/sites/wordpress.sebastien-deren.fr/wp-content/plugins/wp-optimize/cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://wordpress.sebastien-deren.fr/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', '/home/clients/550ab55aa9f1af60db401dfade345bcc/sites/wordpress.sebastien-deren.fr/wp-content/plugins/wp-optimize/cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-wordpress.sebastien-deren.fr.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-wordpress.sebastien-deren.fr.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-wordpress.sebastien-deren.fr.php');
}
